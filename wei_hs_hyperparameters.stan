...
model {
  ...
  //target += weibull_ccdf_log(ycen, alpha, exp(-(mu + Xcen_bg * beta_bg + Xcen_biom * beta_biom)/alpha));
  target += weibull_lccdf(
    ycen | alpha, exp(-(mu + Xcen_bg * beta_bg + Xcen_biom * beta_biom)/alpha));

  beta_biom_raw ~ normal(beta_biom_raw_prior_loc, beta_biom_raw_prior_scale);
  beta_bg_raw ~ normal(beta_bg_raw_prior_loc, beta_bg_raw_prior_scale);
  alpha_raw ~ normal(alpha_raw_prior_loc, alpha_raw_prior_scale);

  mu ~ normal(mu_prior_loc, tau_mu);
}
